...
| B33 | Orchestrator | 🟡 Medium | **`searchSingleQuery` blocks on slowest source.** `wg.Wait()` waits for ALL parallel source searches, not just the first result. With `download_order: ["deezer", "soulseek"]` and slskd unreachable (10s timeout), every single pending track resolution in `resolvePendingSources` takes 10 seconds — even though Deezer returns in 200ms. 20-track playlist import = 200s of waiting. Fix: cancel remaining goroutines after preferred source returns results, or use `context.WithTimeout` per query. Found during Phase 1 album architecture work. |
