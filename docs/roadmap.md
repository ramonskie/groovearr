...
| B33 | Orchestrator | ✅ Done | **`searchSingleQuery` blocks on slowest source.** `wg.Wait()` waited for ALL parallel source searches, not just the first result. Fixed by converting to sequential fallback: providers tried one at a time in `download_order` priority. First provider returning results ≥ `minMatchConfidence` (0.55) wins — remaining providers skipped. Error on non-preferred provider now `Warn`-level (not `Error`) since fallback is expected. |
