...
export default function DiscogsSection({ source }: Props) {
  const {
    register,
    formState: { errors },
  ...
  const handleTest = useCallback(async () => {
    setTestResult(null);
    try {
      await testConnection.mutateAsync("discogs");
    ...
    >
      <div className="space-y-4">
        <FormGroup
          ...
          htmlFor="discogs_consumer_key"
          hint="Discogs API consumer key. Leave empty for anonymous access (lower rate limits)."
          error={errors.discogs_consumer_key?.message}
        >
          <input
            id="discogs_consumer_key"
            type="text"
            placeholder="Consumer key"
            {...register("discogs_consumer_key")}
        ...
        <FormGroup
          ...
          htmlFor="discogs_consumer_secret"
          hint="Discogs API consumer secret. Only needed with consumer key."
          error={errors.discogs_consumer_secret?.message}
        >
          <input
            id="discogs_consumer_secret"
            type="password"
            placeholder="Consumer secret"
            {...register("discogs_consumer_secret")}
            className="w-full rounded-lg border border-slate-700 bg-slate-800 px-3 py-2 text-sm text-white placeholder:text-slate-500 focus:border-purple-500 focus:outline-none focus:ring-1 focus:ring-purple-500"
          />
        </FormGroup>
      </div>
