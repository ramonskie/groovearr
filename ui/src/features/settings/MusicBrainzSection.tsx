...
export default function MusicBrainzSection({ source }: Props) {
  const {
    register,
    formState: { errors },
  ...
  const handleTest = useCallback(async () => {
    setTestResult(null);
    try {
      await testConnection.mutateAsync("musicbrainz");
    ...
    >
      <div className="space-y-4">
        <FormGroup
          ...
          htmlFor="musicbrainz_email"
          hint="Used in the User-Agent header per MusicBrainz API guidelines. Leave empty for anonymous access."
          error={errors.musicbrainz_email?.message}
        >
          <input
            id="musicbrainz_email"
            type="email"
            placeholder="you@example.com"
            {...register("musicbrainz_email")}
